<template>
  ...
  >
    <el-table-column label="日期1" width="180">
      <template slot-scope="scope">
        <template v-if="scope.row.aa">
          <el-checkbox
            v-for="node in scope.row.aa"
            :key="node.id"
            v-model="node.val"
            @change="(val) => handelChange(node.id, scope.row, 'aa')"
    ...
    <el-table-column label="日期2" width="180" class="col-2">
      <template slot-scope="scope">
        <template v-if="scope.row.bb">
          <el-checkbox
            v-for="node in scope.row.bb"
            :key="node.id"
            v-model="node.val"
            @change="(val) => handelChange(node.id, scope.row, 'bb')"
            >{{ node.label }}</el-checkbox
          >
        </template>
        <el-checkbox v-else v-model="scope.row.aa">备选项2</el-checkbox>
    ...
    <el-table-column label="日期3" width="180">
      <template slot-scope="scope">
        <template v-if="scope.row.cc">
          <el-checkbox
            v-for="node in scope.row.cc"
            :key="node.id"
            v-model="node.val"
            @change="(val) => handelChange(node.id, scope.row, 'cc')"
            >{{ node.label }}</el-checkbox
          >
        </template>
        <el-checkbox v-else v-model="scope.row.aa">备选项2</el-checkbox>
    ...
    <el-table-column label="日期4" width="180">
      <template slot-scope="scope">
        <template v-if="scope.row.dd">
          <el-checkbox
            v-for="node in scope.row.dd"
            :key="node.id"
            v-model="node.val"
            @change="(val) => handelChange(node.id, scope.row, 'dd')"
            >{{ node.label }}</el-checkbox
          >
        </template>
        <el-checkbox v-else v-model="scope.row.aa">备选项2</el-checkbox>
    ...
    toChildrenCheck(curArr, row, id) {
      curArr.forEach((ele) => {
        if (ele.id === id && ele.children) {
          ...
          row[prop].forEach((son) => {
            // 子级数组处理
            if (idArr.includes(son.id)) {
              // 触发节点的子节点
              if (ele.val) {
                son.val = true;
              } else {
