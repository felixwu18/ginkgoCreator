<template>
  <div class="about com_border">
    ...
    <h2>健壮性处理</h2>
    <el-button @click="handleAdapter">点击数据适配</el-button><br /><br />
    <el-button @click="handleRequire"
      >点击测试require.context批量导入</el-button
    >
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    handleAdapter(a = {}) {
      // console.log(mri(["-foo", "bar"]), "<=== 111111");
      // var temp = mri(["--foo", "bar"], {
    ...
    handleRequire(a = {}) {
      ...
      function importAll(r) {
        console.log(r.keys(), "<=== rrrrr1111");
        // if(!(key)) { console.log(key);debugger }
        // key.slice(2, -5)
        // r.keys().forEach((key) => fileNames.push(key.slice(2, -5)));
        // import { a } from `./modules/1.js`
        const ttt = require(`./modules/1.js`);
        console.log(ttt, "<=== 111111-out");
        r.keys().forEach((key) => {
          // import ceshi from `./modules/${key.slice(2)}`
          // console.log(ceshi, "<=== 111111");
          // console.log(`./modules/${key.slice(2)}`, "<=== 111111");
        });
      }
